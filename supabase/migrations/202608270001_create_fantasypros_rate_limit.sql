-- Tracks authenticated FantasyPros draft requests for server-enforced rate limiting.

create table private.fantasypros_import_requests (
  id bigint generated always as identity primary key,
  user_id uuid not null references auth.users(id) on delete cascade,
  mock_draft_key text not null check (
    char_length(btrim(mock_draft_key)) > 0
  ),
  requested_at timestamptz not null default now()
);

alter table private.fantasypros_import_requests
enable row level security;

revoke all
on table private.fantasypros_import_requests
from public, anon, authenticated;

create index fantasypros_import_requests_user_time_idx
on private.fantasypros_import_requests (
  user_id,
  requested_at desc
);

create index fantasypros_import_requests_user_key_time_idx
on private.fantasypros_import_requests (
  user_id,
  mock_draft_key,
  requested_at desc
...
revoke all
on function public.check_fantasypros_import_rate_limit(text)
from public, anon;
